-- Enhanced Connections and Company Data Schema
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'user_privacy_settings' AND column_name = 'show_connections_to'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'user_privacy_settings' AND column_name = 'show_connection_count'
  ) THEN
    ALTER TABLE public.user_privacy_settings
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'connections' AND column_name = 'metadata'
  ) THEN
...
CREATE POLICY "Only admins can insert companies" ON public.companies
  FOR INSERT WITH CHECK (false); -- Will be done via API
